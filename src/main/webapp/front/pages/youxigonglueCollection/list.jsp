            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">攻略收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="youxigonglueName" id="youxigonglueName" placeholder="标题" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="youxigonglueTypes" id="youxigonglueTypes" lay-filter="youxigonglueTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in youxigonglueTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../youxigonglue/detail.jsp?id='+item.youxigonglueId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.youxigongluePhoto?item.youxigongluePhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../youxigonglue/detail.jsp?id='+item.youxigonglueId)"><span>{{item.youxigonglueName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteYouxigonglueCollection(item.id)">删除 {{item.youxigonglueName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 youxigonglueCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            youxigonglueCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteYouxigonglueCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('youxigonglueCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#youxigonglueName').val() != null && jquery('#youxigonglueName').val() != "" && jquery('#youxigonglueName').val() != "null" ) {
                param['youxigonglueName'] = jquery('#youxigonglueName').val();
            }
                 
            var youxigonglueTypes = $("#youxigonglueTypes").find("option:selected").val();
            if (youxigonglueTypes != null && youxigonglueTypes != "" && youxigonglueTypes != "null") {
                            param['youxigonglueTypes'] = youxigonglueTypes;
                        }
                                                                                
            // 当前表
                                // 获取列表数据
            http.request('youxigonglueCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function youxigonglueTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youxigonglue_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.youxigonglueTypesList = res.data.list;
               }
           });
       }
